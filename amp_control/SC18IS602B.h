...
public:
     ...
     * */
    bool spiTransfer(int slaveNum, uint8_t* txData, size_t txLen, uint8_t* readBuf);
 
     /* Executes a SPI transfer.
     * Slave select number `slaveNum` will be used.
     * The txLen bytes from the txData buffer will be sent,
     * 
     * then rxLen bytes will be read into readBuf.
     * Returns success (true/false).
     * */   
    bool spiTransfer(int slaveNum, uint8_t* txData, size_t txLen, uint8_t* readBuf, size_t rxLen);
